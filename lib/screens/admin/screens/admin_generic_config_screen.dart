...
class _AdminGenericConfigScreenState extends State<AdminGenericConfigScreen> {
  ...
  void _delete(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
        title: Text('Delete Entry'.tr, style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
        content: Text('Are you sure you want to delete this entry?'.tr),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text('Cancel'.tr)),
          ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: Colors.red, foregroundColor: Colors.white), onPressed: () { setState(() => _data.removeAt(index)); Navigator.pop(ctx); }, child: Text('Delete'.tr)),
        ],
      ),
    );
  }
      ...
      body: Column(children: [
        Container(
          ...
          child: Center(child: Column(children: [
            Text(widget.title, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFFB45309))),
            SizedBox(height: 12),
            ElevatedButton.icon(
            ...
            child: Container(
              ...
              child: Column(children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: Color(0xFF2D3748), borderRadius: BorderRadius.only(topLeft: Radius.circular(12), topRight: Radius.circular(12))),
                  child: Row(children: [
                    ...widget.columns.map((c) => Expanded(child: Text(c, style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)))),
                    SizedBox(width: 80),
                  ]),
                ),
                _data.isEmpty
                    ? Padding(
                        ...
                        child: Text('No ${widget.title.toLowerCase()} entries yet', style: TextStyle(color: Colors.grey)),
                      )
                    : Column(children: _data.asMap().entries.map((e) {
                        ...
                        return Column(children: [
                          Padding(
                            padding: EdgeInsets.symmetric(horizontal: 16, vertical: 12),
                            child: Row(children: [
                              ...widget.columns.map((c) => Expanded(child: Text(row[c] ?? '', style: TextStyle(fontSize: 13)))),
                              Row(children: [
                                InkWell(onTap: () => _showAddDialog(editIndex: i), child: Container(width: 32, height: 32, decoration: BoxDecoration(color: Color(0xFF2563EB), shape: BoxShape.circle), child: Icon(Icons.edit, color: Colors.white, size: 16))),
                                SizedBox(width: 8),
                ...
                Container(
                  ...
                  decoration: BoxDecoration(color: Color(0xFFFEF3C7), borderRadius: BorderRadius.only(bottomLeft: Radius.circular(12), bottomRight: Radius.circular(12))),
                  child: Row(children: [
                    Text('Items per page:'.tr, style: TextStyle(fontSize: 12, color: Colors.black54)),
                    SizedBox(width: 8),
                    Container(padding: EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(border: Border.all(color: Colors.grey.shade400), borderRadius: BorderRadius.circular(4), color: Colors.white), child: Text('10'.tr, style: TextStyle(fontSize: 12))),
                    SizedBox(width: 16),
                    Text('1 - ${_data.length} of ${_data.length}', style: TextStyle(fontSize: 12, color: Colors.black54)),
                  ]),
                ),
              ]),
            ),
